...
- SOC 2 Type II Compliance: Financial advisory client data (AUM, holdings,
  tax brackets) is subject to SEC Regulation S-P. All endpoints must enforce
  RBAC via Clerk or Auth0 — advisors see only their assigned book of clients.
- Field-Level Encryption: Sensitive fields (SSN, account numbers, net worth)
  should use application-layer encryption (AWS KMS envelope encryption) so
  database compromises don't expose PII. The briefing generator should only
  decrypt fields it needs for the current operation.
- Audit Logging: SEC Rule 17a-4 requires immutable audit trails for client
  data access. Log every briefing generation and data access event to an
  append-only store (e.g., S3 + Athena or Datadog Logs with tamper detection).
- LLM Output Review: AI-generated briefings touching investment advice must
  carry a "generated by AI — advisor review required" disclaimer and should
  never contain specific buy/sell recommendations without human approval.
"""
